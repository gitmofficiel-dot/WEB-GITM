...
import { FileEdit, Image as ImageIcon, MessageSquare, Plus, Edit2, Trash2, Bot, Sparkles, Send } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {[
          { label: 'Published News', val: '24', icon: FileEdit, color: 'text-blue-500', bg: 'bg-blue-500/10' },
          { label: 'Gallery Items', val: '156', icon: ImageIcon, color: 'text-purple-500', bg: 'bg-purple-500/10' },
      ...
      <div className="glass-card rounded-2xl p-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {['New AI Lab Opening', 'Summer Hackathon 2026', 'Interview with President'].map((item, i) => (
                <tr key={i} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  <td className="p-3 font-medium text-slate-800 dark:text-white">{item}</td>
                  <td className="p-3 text-sm text-slate-500">{i === 1 ? 'Event' : 'News'}</td>
                  <td className="p-3">
                    <span className="px-2 py-1 text-[10px] uppercase font-bold rounded bg-emerald-100 text-emerald-600">Published</span>
                  </td>
                  <td className="p-3 text-sm text-slate-500">June 1{i}, 2026</td>
                  <td className="p-3 text-right">
                    <div className="flex justify-end gap-2">
                      <button className="p-2 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded"><Edit2 size={14}/></button>
                      <button className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded"><Trash2 size={14}/></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {/* AI Content Writer */}
      <div className="glass-card rounded-2xl p-6 border-purple-500/30 shadow-[0_0_20px_rgba(168,85,247,0.15)] relative overflow-hidden mt-6">
        ...
        <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border border-purple-500/20">
          <textarea 
            ...
            className="w-full h-20 bg-transparent border border-slate-300 dark:border-slate-700 rounded-lg p-3 outline-none text-sm resize-none focus:border-purple-500 mb-3"
          ></textarea>
          <div className="flex justify-end gap-2">
            <button className="px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white rounded-lg text-sm font-bold shadow-lg shadow-purple-500/20 transition-all flex items-center gap-2">
              <Sparkles size={16} /> {txt(lang, 'Generate Article', 'توليد المقال', 'Générer', '生成文章')}
            </button>
          </div>
        </div>
      </div>
